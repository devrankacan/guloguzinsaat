...
export default function PortfolioManager({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    data.set("completedAt", form.completedAt);
    ...
    try {
      ...
      setItems((prev) => {
        if (editingId) {
          return prev.map((p) => (p.id === editingId ? result.item : p));
        }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink/60">{items.length} proje kayıtlı.</p>
      ...
      {formOpen && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 bg-white p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold uppercase text-ink/60">Başlık</label>
            <input
              type="text"
              required
              value={form.title}
              onChange={(e) => setForm({ ...form, title: e.target.value })}
              placeholder="Yakutiye Villa Yıkımı"
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase text-ink/60">Kategori</label>
              <input
                type="text"
                value={form.category}
                onChange={(e) => setForm({ ...form, category: e.target.value })}
                placeholder="İnşaat / Yıkım / Hafriyat"
                className="border border-ink/15 px-4 py-2.5 text-sm focus:border-gold focus:outline-none"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase text-ink/60">Konum</label>
              <input
                type="text"
                value={form.location}
                onChange={(e) => setForm({ ...form, location: e.target.value })}
                placeholder="Yakutiye/Erzurum"
                className="border border-ink/15 px-4 py-2.5 text-sm focus:border-gold focus:outline-none"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase text-ink/60">Tamamlanma</label>
              <input
                type="text"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold uppercase text-ink/60">Açıklama</label>
            <textarea
          ...
          {existingImages.length > 0 && (
            <div>
              <label className="text-xs font-bold uppercase text-ink/60">Mevcut Görseller</label>
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold uppercase text-ink/60">Görsel Ekle</label>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
      ...
      <div className="flex flex-col gap-3">
        {items.length === 0 && (
          <p className="bg-white p-6 text-sm text-ink/60 shadow-sm">Henüz proje eklenmedi.</p>
        )}
        {items.map((item) => (
          <div
            key={item.id}
